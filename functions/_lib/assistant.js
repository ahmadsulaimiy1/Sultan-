// THE ASSISTANT'S BRAIN — shared by every channel it speaks through.
//
// This began inside functions/api/chat.js, which serves the website's
// chat panel. WhatsApp now answers with the same assistant, and the one
// thing that must never be copy-pasted is SITE_FACTS: it is the list of
// things this school is permitted to state to a parent, and two copies
// of it would drift the moment somebody corrected one of them. A fact
// sheet that disagrees with itself is worse than no fact sheet, so it
// lives here once and both channels import it.
//
// Nothing about the website's behaviour changes by moving it: chat.js
// imports exactly what it used to declare, and buildSystemPrompt's new
...
export const MAX_IMAGE_BLOCKS_PER_MESSAGE = 3;
...
1. Sultan Hanafi Nursery and Primary School — ages 2-10, day. Nigerian curriculum blended with Islamic teachings. Head Teacher: Mrs. Mariam Tope Anofi-AbdulKareem (B.Sc. Microbiology, B.Ed, PGDE, NCE), who is also the Registrar of Royal College.
...
export async function retrieveRelevantPages(request, lang, userMessage) {
  try {
...
export function buildSystemPrompt(lang, office, style, groundingPages, channel) {
  ...
  // WhatsApp is a different room from the website panel: no markdown
  // renders there, the reply arrives as a phone notification, and a wall
  // of text is unreadable on a small screen. Omitted for the web panel,
  // so nothing about the existing channel changes.
