...
const BOOL_FIELDS = [
  'channel_website', 'channel_email', 'channel_whatsapp', 'channel_sms',
...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    return json({
      preferences: {
        channel_website: true, channel_email: false, channel_whatsapp: false, channel_sms: false,
  ...
  try {
    await sql`
      INSERT INTO guardian_notification_preferences (
        guardian_id, channel_website, channel_email, channel_whatsapp, channel_sms,
        type_attendance, type_results, type_fees, type_announcements, type_events, type_emergency,
        language, updated_at
      ) VALUES (
        ${session.guardianId}, ${values.channel_website}, ${values.channel_email}, ${values.channel_whatsapp}, ${values.channel_sms},
      ...
      ON CONFLICT (guardian_id) DO UPDATE SET
        ...
        channel_whatsapp = EXCLUDED.channel_whatsapp, channel_sms = EXCLUDED.channel_sms,
