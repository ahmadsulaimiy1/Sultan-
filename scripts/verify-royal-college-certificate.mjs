...
// A programme may confer more than one award. Every award this batch actually
// made must appear on the print file — checking only the first would let a
// second variant go unverified, and the second variant is exactly the thing
// most likely to be wrong.
const AWARDS = Array.isArray(reg.award) ? reg.award : [reg.award];
const TITLES = PROG.variants
  ? [...new Set(Object.values(PROG.variants).map((v) => v.title))]
  : [PROG.title];
const MUST_APPEAR = [
  'Sultan Hanafi Royal Schools', PROG.school,
  ...TITLES, ...AWARDS,
  'Federal Republic of Nigeria', PROG.signatory.name,
  'Dr. Zakariya Olanrewaju Anofi', 'Student Identity Number',
];
...
// Any Arabic at all, not just the phrases above — EXCEPT strings a human
// approved for this specific programme.
//
// The blanket ban came from the Founder's 2026-08-06 directive that the Royal
// College sheets are English only, and it stands for those. It cannot stand
// unchanged for the Qur'an College, where he supplied Arabic himself: the
// College's motto is transcribed off its own crest, and one graduate's name
// was written out for engraving in his own message. Deleting the check to let
// those through would have thrown away the thing it exists to catch — Arabic
// that no human ever approved, generated by transliterating a name or by
// copying a phrase out of the specimen artwork.
//
// So the rule becomes a whitelist rather than a ban. Every run of Arabic on
// the sheet is extracted and matched against the programme's declared motto
// and the Arabic names carried in this batch's own register. Anything else —
// one word, one stray fragment of the specimen — still fails the batch.
{
  const plain = printHtml.replace(/&#x[0-9A-Fa-f]+;/g, '');
  // The whitelist: the programme's declared Arabic (transcribed from the frozen
  // master and from the College's own crest), the Arabic of every award this
  // batch confers, and the Arabic names a human supplied for this roll. The
  // citation sentence is assembled from those parts at render time, so it is
  // matched by its pieces rather than as one string.
  const parts = [
    PROG.mottoAr,
    ...
    'قد أتمّ بنجاحٍ', 'قد أتمّت بنجاحٍ', 'في العام الدراسي',
    'وفقًا للمناهج المعتمدة والمعايير الأكاديمية المعمول بها في الكلية،',
  ].filter(Boolean).map((t) => t.replace(/\s+/g, ' ').trim());
  // Matched WORD by word, not run by run. Punctuation splits the rendered text
  // into runs that straddle two approved strings — the nation line is one
  // declared string with a middle dot in it, and the citation is assembled from
  // a connective plus a stage clause — so run-level matching rejected wording
  // that is entirely approved. Word level is the granularity that actually
  // answers the question this gate asks: is there a word on this sheet that no
  // human wrote? An invented word, a transliterated name or a phrase lifted
  // from the specimen artwork still fails, because none of their words are in
  // the corpus.
  // Split on anything that is not an Arabic LETTER or one of its diacritics.
  // The Arabic comma and full stop live inside the same Unicode block as the
  // letters, so a naive block-range split makes "،" a word of its own and the
  // gate then reports the sheet's punctuation as unapproved vocabulary.
  const words = (t) => String(t)
    .split(/[^\u0620-\u064A\u064B-\u0652\u0670\u0671-\u06D3\u06FA-\u06FF]+/)
    .filter((w) => /[\u0620-\u064A\u0671-\u06D3]/.test(w));
  const corpus = new Set(parts.flatMap(words));
  const runs = [...new Set((plain.match(/[؀-ۿݐ-ݿﭐ-﷿ﹰ-﻿][؀-ۿݐ-ݿﭐ-﷿ﹰ-﻿\s]*/g) || [])
    .map((t) => t.replace(/\s+/g, ' ').trim()).filter(Boolean))];
  const unapproved = [...new Set(runs.flatMap(words).filter((w) => !corpus.has(w)))];
  check('every Arabic string on the sheet was written by a human and approved',
    unapproved.length === 0,
    unapproved.length ? `words on the sheet from no approved source: ${unapproved.join(' · ')}` : '');
  if (runs.length) console.log(`       ${new Set(runs.flatMap(words)).size} distinct Arabic words, every one from an approved source`);
}
