...
cannot draw the bracketed frame, the rail or the medallion. So the whole
ground is placed as one image, anchored to the *page* rather than to the
text and set behind it.

NOTE ON PAGINATION. Word applies one header to every page, so this file
carries the single-sheet form — header and footer both. For a letter that
runs to more than one sheet, use the generator, which applies the
correspondence standard automatically; see
docs/shrs-correspondence-standard.md. That is the only construction Word honours full-bleed: an inline
...
HEAD_TOP, FOOT_H = 66, 60    # where the typist's text may begin and end             # the two bands, as identity.py draws them
...
DOCUMENT = (
  ...
  f' w:left="{twip(30)}" w:header="0" w:footer="0" w:gutter="0"/>'
  '<w:titlePg w:val="0"/>'
...
STYLES = (
  ...
  '<w:color w:val="2B2118"/><w:sz w:val="21"/><w:szCs w:val="21"/>'
