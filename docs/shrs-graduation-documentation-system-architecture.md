...
- **Student/guardian data model** — `students`, `guardians`, `guardian_student`, `student_classes` (dual/multi-enrolment — already handles a student in both Secular College JSS3 and Islamiyyah College, exactly the real case in this cohort), `term_results`, `student_lifecycle_events` (with `event_type = 'graduation'` already defined, though not yet gated by anything).
...
Checked against the real cohort (36 named students across four groups, several dual-enrolled between Secular College and Islamiyyah College, per the existing `student_classes` model):

| Document | Basic School (BASIC) | Secular College JSS3 | Secular College SSS3 | Islamiyyah College (ISLAMIYYAH) |
|---|---|---|---|---|
...
All three signals are now wired into `graduation-clearances.js`'s `?recordId=` response and rendered as warning chips in the Graduation Control Centre timeline (`renderSignals()` in `js/portal-graduation-control.js`) — every case entered today, even manually, immediately makes the corresponding clearance stage decision-with-information rather than a blind sign-off. Data entry into all three registers is now reachable from the same screen: the Control Centre's record detail view carries three inline "+ Record…" forms (`renderRegisterForms()` in `js/portal-graduation-control.js`) that post directly to `disciplinary-cases.js`/`library-loans.js`/`issued-devices.js` and refresh the signal panel on success — a staff member reviewing a record's clearance no longer has to leave the page to feed the register that informs it. Done, front-to-back.

### 9.4 Item 6 — Dashboard Integration

The generic Office Portal endpoint (`functions/api/portal/staff/office/[slug].js`) now computes a `graduationQueue` for any office that is a real decision point in `STAGE_DEFINITIONS` — Academic Affairs, Examinations, Finance, Library, Digital Services (ICT), and the Head Teacher/Principal/Ra'ees/Mudeer offices (via the `principal` stage) and the Executive office (via the `founder` stage) — reusing a new `queueForStageCodes()` helper in `graduation-workflow.js` that applies the exact same "currently-actionable stage" rule the central Control Centre's own queue uses, just pre-filtered to that office's stage(s). The Workflow Centre tab (`js/portal-office.js`'s `renderWorkflow()`) now renders this queue inline, with a direct link into the Graduation Control Centre for the actual decision, and the office header's pending-workflow count includes it. The stale "Graduation Readiness — no system exists yet" line in the Secular College Principal's `NOT_YET_TRACKED` list was removed, since it is no longer true. **Gap, stated plainly:** VPAC/VPAD have no dedicated office in this codebase (they are role appointments, not office seats) and the Registry stage is auto-cleared, not manually decided — so those two continue to surface only via the central Control Centre, honestly, rather than an office page that doesn't exist. Actually *deciding* a stage still happens on the Control Centre page (the office dashboard link opens it) — a fully inline decide-in-place control on the office page itself was judged unnecessary duplication of a working UI and was not built.

### 9.5 Item 7 — Digital Signatures: architecture, self-managed

`staff_signatures` (schema + `setup.js`) — one row per staff member, either a typed name in a script-style font or a small (≤200KB) base64 image, plus an optional title line, upserted via `functions/api/portal/staff/my-signature.js`. Deliberately self-service and self-scoped: `staff_id` is always read from the session, never a request body parameter, so no staff member can set or view another's signature through this endpoint. No signature image is hardcoded anywhere. A "My Digital Signature" card on the My Identity page (`portal/staff/identity/index.html` + `js/portal-staff-identity.js`) now lets a staff member choose typed or uploaded, preview it live, and save. Done, front-to-back. Whether/how a stored signature is eligible to actually appear on a Stage 3 document remains explicitly a Stage 3 decision, not decided here.
...
- `node --check` on every new and modified JavaScript file (all pass), across both the initial backend-and-signals pass and the follow-on UI pass (Approval Matrix screen, register data-entry forms, signature panel, bulk controls).
- `node scripts/build.js` — the public site build is unaffected, re-confirmed after the UI pass.
- HTML tag-balance checks on every new/modified page (`graduation-control/index.html`, `approval-matrix/index.html`, `identity/index.html`).
...
- The new UI (Approval Matrix screen, register data-entry forms, signature panel, bulk-decision bar) was reviewed against its data contracts and each endpoint's actual request/response shape, but not re-run through a fresh Playwright pass in this session — flagged here rather than silently assumed equivalent to the §8.9 pass.

### 9.10 Honest summary — what Stage 2.5 actually closes

All ten items now have real, working backend architecture *and* a real staff-facing UI surface, front-to-back:

1. VP vacancy — visible in the Control Centre timeline.
2. Approval Matrix — `/portal/staff/approval-matrix/` admin screen.
3–5. Disciplinary/Library/ICT — signals shown, and inline "+ Record…" forms in the Control Centre feed each register from the same screen a decision-maker is already looking at.
6. Dashboard Integration — every mapped office sees its own graduation queue in its Workflow Centre tab.
7. Digital Signatures — "My Digital Signature" card on the My Identity page.
8. Audit Trail — IP/user-agent/before-after values on every write path this stage touched.
9. Batch Operations — bulk approval has a real UI (checkboxes + bulk-action bar in the Control Centre); batch *numbering* and Stage-3-only concepts (bulk printing, bulk verification) remain out of scope by design, not oversight.
10. Stage 3 Document Specification — delivered as `docs/shrs-graduation-document-specification.md`, since superseded by the far more exhaustive `docs/shrs-master-graduation-document-specification.md` (see §11 below).

**Named limits that remain, honestly:** the Founder/VPAC/VPAD stages still have no dedicated office page of their own (they are role appointments, not office seats, so they surface via the central Control Centre and, for Founder, the Executive office's Workflow Centre tab — not a bespoke page); batch numbering has no endpoint yet (deferred to Stage 3, where it has an actual purpose); database-level anti-tampering on `staff_audit_log` is still not enforced at the Postgres role level (carried forward from §8.6/§9.6 unchanged). None of these block Stage 3 architecturally.

Stage 2.5 is complete for the items above, on the same honest terms Stage 2 was.

## 11. Stage 3 Authorization and the Master Graduation Document Specification

The client reviewed Stage 2.5 and issued Executive Authorization — Stage 3, with a standing instruction that this stage — the permanent academic identity system the world will actually see — be the most rigorous in the entire project. Per that authorization's own sequencing ("no document generation should begin until this specification is internally complete"), the deliverable produced first is `docs/shrs-master-graduation-document-specification.md` — a complete, internally-audited standard covering the full 19-document ecosystem (including three internationally-recognized additions identified during drafting: the Diploma Supplement, the Provisional Certificate, and a formal Certified True Copy/Duplicate reissuance protocol), security architecture, numbering, verification platform design, PDF engine constraints (including the load-bearing finding that Cloudflare Pages Functions cannot run headless Chromium/Ghostscript/LibreOffice, and the correct triggered/batch rendering architecture that follows from it), international print standards, the institution's own existing brand palette confirmed as already matching the client's named colour scheme, typography, paper/seal/signature standards, and a ten-audit quality-assurance framework with a hard release gate.

That document supersedes `docs/shrs-graduation-document-specification.md` (the Stage 2.5 draft) as the governing standard. Stage 3 engineering work follows the phased build plan in the master specification's §20 once the two open decisions in its §22 sign-off block (PDF-rendering infrastructure choice; institutional seal asset) are resolved.
