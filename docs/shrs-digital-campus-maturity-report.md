...
| **Identity** | 2 — Developing | Three real, working session systems (guardian/student/staff); a data-driven Permission Engine. The Founder Dashboard's auth migration (bearer token → staff session + `EXE` Permission Engine grant) is now code-complete and verified locally — a genuine, tested improvement, not a design note. Still held at 2, not 3, because almost no real named person has an actual account anywhere reachable today — the Role & Permission Matrix names two real established individuals (CEO, Registrar) and neither has a real `staff` row yet, and the bearer-token fallback cannot be retired until one does. |
...
| **Infrastructure** | 1 — Initial | Every Pages Function is written and proven to run under `wrangler pages dev` against a local database. But no Cloudflare Pages project, no Neon production database, no CI/CD pipeline, and no confirmed domain exist anywhere reachable — confirmed by this session's own blocked attempt to reach the retired `.ng` domain. Code readiness does not raise this score; operational existence does, and there is none. |
