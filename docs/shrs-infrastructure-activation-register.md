...
| Name | Purpose | Priority | Cost | Owner | Dependencies | Status |
|---|---|---|---|---|---|---|
| Executive Identity | Individually-accountable EXE/PRIN/REG logins, replacing the shared bearer token | High | Free — no new infra | ICT / Head of Schools / Administrator's Office | Real staff data (Human Capital Register) + a reachable database | **In Progress** — Founder Dashboard code migrated and verified locally this engagement; no real account exists in any reachable environment yet |
...
| Name | Purpose | Priority | Cost | Owner | Dependencies | Status |
|---|---|---|---|---|---|---|
| MFA | Risk-based login protection, all three login roles | High | Free — reuses the Resend email infrastructure already live, no new vendor | ICT / Cybersecurity | Resend (done) | **Completed and verified live 2026-07-28** — email OTP as a second step after password, on all three logins (guardian/student/staff), proven end-to-end against the real Preview database: real password → real emailed 6-digit code → real portal access. Guardians are covered unconditionally; students/staff activate per-account once ICT enters an email via the admin endpoints. Also shipped the same day: a signed 7-day "trusted device" cookie so OTP doesn't repeat on every login (revoked automatically on password change); a hard gate blocking all dashboard content — not just the admissions-application action — until a new guardian's email is verified; dual-method verification (every OTP/verification email now carries both a typed code and a one-click link, neither a dead end if the other's attempts run out); full (non-masked) email display everywhere it's shown back to the account owner; and a universal password show/hide toggle + live strength meter + password-manager-compatible `autocomplete` across every password field site-wide. See `docs/identity-authentication-roadmap.md` for the full model and what's explicitly deferred (passkeys/WebAuthn, magic links, a real device registry, geo/risk scoring). |
...
| Founder Dashboard | Institution-wide read-only aggregate view | High | Free | Head of Schools / Administrator's Office | DB + hosting live | Completed (code, incl. Executive Identity migration), Not Started (production use) |
...
| LMS | Courses/modules/lessons/assessments/content authoring | Low — explicitly deferred | Unknown/TBD — needs a real content-authoring and hosting budget decision before scoping | Board / Head of Schools / Administrator's Office | Everything above + a content strategy decision | Not Started |
