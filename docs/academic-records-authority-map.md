...
| **Hifz Records** | Muhaffiz/Muhaffizah (`hifz_records` `C`, 'own assigned students') — `admin/hifz-progress.js`, **still bearer-token (`PORTAL_QURAN_TOKEN`), not yet migrated — Migration Phase D** | Not named | Qur'an College Officer + Principal jointly for stage advancement (Matrix `A`) — recordable in the sense that the same bearer-token call performs the stage update; there is no separate approval step or second-signer field at all, not even an optional one | N/A | Same bearer-token endpoint, in-place upsert — no history of prior notes kept | Registrar ('snapshot only', `V` not `X` — the Matrix never grants an explicit Export permission here either) |
| **Ijazah Records** | Qur'an College Officer (`ijazah_records` `C`) — same bearer-token endpoint, action `ijazah.grant`, **not yet migrated — Migration Phase D** | Not named | **Matrix says** Principal + QC-OFF jointly (`A`). **Code says**: the `ijazah.grant` request shape has no field at all for a second signer's ID — not even the optional pattern used elsewhere. This is a stricter gap than "recordable, not enforced": there is nowhere to record it even if someone wanted to. | N/A — granted, therefore it exists; a guardian/student sees it via their own dashboard's Ijazah panel (already built) | **Structurally impossible** — no update path exists for grant fields, only `revoke` (IQ-02 §7.6, enforced at the schema level: `ON DELETE SET NULL`, frozen `student_full_name`) | **Gap**: no Export (`X`) permission exists in the Matrix for Ijazah either — Registrar and QC-OFF hold `V` only |
...
4. **"Recordable, not enforced" is now a confirmed pattern across four
   record types** (Promotion, Graduation, Certificates, and — worse,
   not even recordable — Ijazah). The Approval Workflow Architecture
   roadmap item exists to close this, deliberately not attempted here.
5. **Hifz and Ijazah remain the two record types furthest behind on
   the Identity Migration Plan** — both still bearer-token-gated,
   explicitly Phase D, not touched by Phases A or B.
