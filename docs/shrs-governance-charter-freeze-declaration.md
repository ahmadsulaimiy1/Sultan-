...
**The Charter's own official website, email address, and all three live QR codes (Website, Email, Contact Page) pointed to the retired `.ng` domain — a domain the Institution's own live site stopped using some time ago in favour of `shroyalschools.com`** (confirmed by checking the actual live codebase: `functions/_lib/email.js`'s `SITE_ORIGIN` constant, and 439 separate `.com` references across every live page, against only 46 stray `.ng` references, all in generation scripts for documents this engagement doesn't currently own — this Charter's own `scripts/generate-constitution-html.js` was one of them).

This is not cosmetic: the QR codes are functional — decoding them with the actual current source images confirmed all three pointed to `https://shroyalschools.com`, `mailto:info@shroyalschools.com`, and `https://shroyalschools.com/contact/`, none of which is the Institution's live domain. A reader scanning the Institution's own supreme governing instrument would have been sent to the wrong address.
...
## 10. Amendment after freeze — Article 78A, the Shariah Advisory Council

The freeze in Section 9 held through Phase 1B and into Phase 2 scoping, where a genuine gap surfaced: no Shariah Advisory Council existed anywhere in the Charter's text, and the client directed that it must appear in the Governance Charter itself. This is a real constitutional amendment, made under Chapter XVIII exactly as this Section 9 said any future change would be — not a reopening of the Phase 1A audit itself. Full account of what was added, why, and how it was verified is in Drafting Note 25 of `docs/shrs-governance-charter-drafting-record.md`. The flagship publication was regenerated (now 76 pages) and the cross-reference integrity check re-run clean. The freeze in Sections 1–9 above otherwise stands unchanged; this amendment does not reopen any of the nine audit points, and no new print, publication, or typography defect was introduced by it (confirmed by the same regenerated-PDF verification, not assumed).
