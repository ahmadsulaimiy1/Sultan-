...
| 8 | `admin/announcements.js` | **Migrated — dual-auth.** Staff session + Permission Engine (`communications` area, checked per-action: `create` C, `update` E, `publish`/`unpublish`/`feature`/`unfeature` P, `archive` Ar) is now the PRIMARY path. `PORTAL_ADMIN_TOKEN` remains a fallback ONLY, same reason as #1 and #4 — no real REG/PRIN/EXE staff account is confirmed to exist in any reachable environment yet. | Retire the bearer-token fallback once a real REG/PRIN/EXE staff account is confirmed working in a real environment. | Done — no further design work needed. | A real REG, PRIN, or EXE staff account in a real environment (still doesn't exist). | Complete (code); the retirement step is Not Started, blocked on real infrastructure existing. | Low-Medium — content-only blast radius (public communications, no PII). This migration also added E and Ar to `communications` in the Matrix itself, which had only ever named C/P — a real governance-document gap, not just a code one (see `role-permission-matrix.md` §4.15). PRIN's "own institution" scope is not enforced at the row level — `announcements` has no `institution_id` column — a named gap, same category as #4's MUH note. |
...
Migrate in the order listed above, because each one's complexity depends
on the previous one's real staff account existing. **#1 (Founder
Dashboard), #4 (Qur'an College), and #8 (Announcements admin) are now
all code-complete and verified locally** — none needed #3's harder
