...
**#1 (Founder Dashboard) is now code-complete and verified locally** —
it did not need #3's harder bootstrap problem solved first, exactly as
predicted. #2 (Qur'an) is the next-lowest-complexity item once real
Muhaffiz/Qur'an College Officer accounts exist; #3 (the admin/sysadmin
bootstrap tokens) remains the hardest because it has no real identity
target to migrate to that doesn't reintroduce its own bootstrap
problem.
