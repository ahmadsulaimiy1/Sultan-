...
**Certificate System v1.3 · programme code `JSS` · English only**
...
| **The ring** | A 20mm border on all four sides in five layers — navy trim hairline, solid gold fillet, crimson lathe ribbon, navy bead-and-reel, engine-turned gold ribbon, inner fillet — **mitred at 45° at every corner** | v1.2's border was one narrow gold band and read, correctly, as thin. v1.3 gives it depth, and gives every band its own printed ground so it reads as a ribbon of colour rather than as lines drawn on paper. The mitre matters structurally: a ring of one depth on all four sides is continuous, so there is no join to cover, and nothing has to be patched over a corner. An earlier v1.3 cut made the sides deeper than the head; that cannot mitre, and the patch it forces is exactly what was rejected. |
| **Three metals, plus crimson and navy** | Throughout | Yellow gold is the working metal, red gold the contrast, pale gold the highlight and counter-line; crimson carries the ribbons and the bosses; navy holds the trim, the bead rule and the pips. One-metal ornament reads flat at any weight. |
| **Corner bosses** | Four struck medallions, one riding each mitre | **These replace v1.2's corner fans and v1.1's navy corner blocks.** Gold rim with 20 struck beads, crimson ground, a pale-gold rosette engraved on the crimson, a navy pip at the centre: all three inks in one piece, at the four points the eye reaches first. It sits *on* a continuous ring, which is what separates a boss from a patch. |
| **Engine-turned ribbon** | A 3.2mm gold band inside the crimson one, all four sides | Interlocking rope arcs with a struck quatrefoil in each cell, emitted as **real vector paths**. An SVG `<pattern>` fill is rasterised into the PDF whatever it contains — that one finding took the press file from 102MB to 8.6MB. |
| **Crimson lathe ribbon** | A 5mm band inside the gold fillet, all four sides | Four crimson strands over a printed blush ground with a red-gold counter-line between them. Two inks in register at hairline weight is the layer a desktop scanner reproduces worst. |
