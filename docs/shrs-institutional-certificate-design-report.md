...
**Status: research delivered; 2 of 3 recommendations approved and implemented, 1 deferred.** The client approved recommendations 1 (typographic unification) and 2 (dimensional rendering) from §4 below, both implemented as craftsmanship-only changes — see `docs/shrs-master-graduation-document-specification.md`'s corresponding build-log entry for the full technical account and Playwright verification (100%/200%/400% zoom, EN+AR). Recommendation 3 (paper-grain/vignette) was explicitly deferred at the client's own request, to be evaluated after typography and dimensional rendering are both final. Every other Category C item named in §2's "elements that should evolve" list remains untouched and frozen pending future approval.
...
1. **Typographic unification — APPROVED, implemented.** Collapse the current four-family mix — and the monospace treatment on the certificate number and verification code — into one disciplined serif type system, varied only by size, weight, and italic.
   ...
   *Implementation:* Cormorant Garamond (English) + Amiri (Arabic) throughout; numerals set with `font-variant-numeric:lining-nums tabular-nums` instead of monospace. See the Master Spec build log for the full account.

2. **Dimensional rendering of the frame moulding, corner ornaments, and seal presentation — APPROVED, implemented.** Replace the current flat-colour SVG treatment with real highlight/core-shadow/cast-shadow rendering, so these elements read as carved, cast, or embossed rather than printed flat shapes.
   ...
   *Implementation:* highlight/shadow bevel strokes on the corner ornament and frame-band tile, layered drop-shadow + inset foil sheen on the seals, subtle lift shadow on the crests — all paint-only (no layout property touched). See the Master Spec build log for the full account.

3. **A simulated paper-grain and soft edge vignette layer — DEFERRED at the client's request.** Honestly documented — exactly the same explicit honesty boundary this project already applies to the hologram strip (a screen/PDF simulation of a physical security feature, not a claim that physical foil exists) — signalling physical cotton/parchment stock even though the document is issued digitally.
   ...
   *Status:* not implemented. The client's stated reasoning — safer to evaluate after typography and dimensional rendering are both final, since an artificial texture added too early can fight with print-stock decisions made later — is sound and is recorded here as the reason, not silently dropped.
