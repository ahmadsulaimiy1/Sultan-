...
> proposed defaults — SHRS's actual current device/network
> infrastructure has not been described to build this section from real
> specifics, and this policy says so rather than inventing a network
> policy for infrastructure that may not exist yet.
...
physical digital infrastructure (school devices, network) — the
...
write specific rules — this section remains open pending real input,
