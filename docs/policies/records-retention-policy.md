# SHRS Records Retention Policy

**Status:** DRAFT v0.1 — not yet adopted. Unlike most documents in this
set, this one is written to actually **resolve** an open item —
"no retention period defined" — that Phase A and Phase B both
correctly named as a gap without filling it, since that wasn't their
job.
**Owner (proposed):** Registrar, Royal College (Mrs.
Anofi-Abdulkareem Mariam Tope), jointly with the ICT Head for
Portal-held data.
**Review cycle:** Annual.
**Depends on:** Data Protection & Privacy Policy (Phase A — once this
policy is adopted, that document's §5/§10 should be updated to point
here instead of restating "no period defined yet"); Ijazah Governance
Framework (Phase A, for the one deliberately indefinite retention
category).
...
document rather than each inventing its own.
...
Data Protection & Privacy Policy without redefining them.

## 4. Principles

1. A record is kept only as long as it serves a genuine purpose —
   defined per category below, not as a blanket "keep everything
   forever" default.
2. Some categories are deliberately long or indefinite where the
   record's value outlives enrolment (an Ijazah register entry, for
   example) — these are named exceptions, not oversights.
3. When a retention period expires, the record is deleted or
   anonymised, not simply left in place because deleting it wasn't
   automated — this is a real operational commitment, not just a
   paper policy, and should be checked at each annual review.

## 5. Proposed retention periods (pending Board confirmation)
...
| Designated Safeguarding Lead (once appointed) | Owns the safeguarding-log retention category specifically, given its sensitivity. |

## 7. Procedures

- At each annual review, the Registrar/ICT Head identify records past
  their retention period and confirm deletion or anonymisation — this
...
data isn't kept longer than necessary — the specific periods above are
proposals to satisfy that principle, not a certified compliance
position; legal confirmation is still required, especially for the
safeguarding-record category.
...
The Ijazah register (Section 5) is the one deliberate indefinite-
...
Privacy Policy's retention-related sections.

## 12. Version control

| Version | Date | Change | Author |
|---|---|---|---|
| 0.1 | Draft | Initial draft from Governance Master Register, Phase C | Drafted per SHRS governance directive; not yet reviewed or adopted; every period is a proposed default pending Board and, for the safeguarding category, professional confirmation |
