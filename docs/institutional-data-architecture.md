...
| Teachers | `staff`, `staff_roles` (`TCH`), `teacher_class_assignments` | Yes | Nothing structural. Zero real teachers exist in the system today (see Phase 5 readiness review). |
...
| Arabic & Islamic Studies programme | `classes` (institution = `Arabic & Islamic Studies`), `student_classes` | Yes, generically | No programme-specific data model beyond generic class enrolment — same class-ladder gap as Royal College/Nursery & Primary. |
...
1. **Institutions and campuses** — already seeded, real, no action needed (`Nursery & Primary`, `Royal College`, `Arabic & Islamic Studies`, `Qur'an College`; `Main Campus — Ikorodu`).
