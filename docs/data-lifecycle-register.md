...
- **Assessment Records still have no Registrar's Office write path.**
  Attendance moved (Migration Phase A, above); results still go through
  `admin/students.js`'s bearer token — Phase B, queued next in
  `identity-migration-plan.md`.
- **Migrating Attendance surfaced a real, previously-hidden operational
  gap rather than closing one cleanly.** The Matrix only grants REG
  Edit on `attendance` ('correction' scope) — Create belongs to a Class
  Teacher role with no issued account. So as of Phase A, no working
  path exists anywhere in this project to enter a brand-new term's
  attendance for the first time; only corrections to a term already on
  file are possible. This is named plainly rather than resolved by
  granting REG a Create permission the Matrix never assigned them.
