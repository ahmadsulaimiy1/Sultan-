...
| **Attendance Records** | `registrar/attendance.js` (single-student corrections, session + Permission Engine) OR **`teacher/attendance.js`** (whole-class bulk, session + Permission Engine + `teacher_class_assignments` check) — as of Teacher Identity & Academic Workforce Activation, both Create and Edit have a real working path. Edit (correcting an existing term's row) is granted to REG/AREG (unscoped) and PRIN (own institution) via the Registrar endpoint; Create (a term's first row) is a Class Teacher (`TCH`) function via the Teacher Portal endpoint, gated on an active `teacher_class_assignments` row naming that exact class, not just holding the `TCH` role somewhere. `admin/students.js`'s old bearer-token path is closed (returns 410). | Class Teacher, via the Teacher Portal; corrections via Registrar / Academic Office | Both endpoints' upsert; still no reasoned-event trail for corrections — an upsert overwrites the prior value with no history kept, unlike `student_lifecycle_events`. | None enforced — Principal correction-oversight is a Matrix statement (§4.x), not a system check on this endpoint. | Registrar, via `registrar/student.js`'s read; teacher, via their own roster | None — overwritten in place, not archived | Bundled with the student record's retention (IT-04 §7.1). Not enforced in code. |
| **Assessment Records** *(raw CA/exam entry, `term_results`)* | `registrar/assessments.js` (single-student corrections) OR **`teacher/assessments.js`** (whole-class bulk, subject-scoped) — same Teacher Identity update as Attendance above. Edit (correcting an existing student/term/subject row) is granted to REG (unscoped, 'correction only, logged') via the Registrar endpoint; Create (a subject's first row for a term) is a Subject Teacher (`TCH` holding a subject-specific `teacher_class_assignments` row) function via the Teacher Portal endpoint. `admin/students.js`'s old bearer-token path is closed (410). **Muhaffiz (`MUH`) and Arabic Instructor (`ARB`) remain unresolved** — both are still `'proposed'` roles with no issued account; `teacher_class_assignments` is schema-ready for them but neither has been onboarded. | Subject Teacher, via the Teacher Portal; corrections via Registrar | Both endpoints' upsert; still no correction-history trail (matches the Ownership Register's note that corrections should stay visible — today they don't, an upsert replaces the row). | None enforced — see the separate Results row below for the Matrix's distinct, also-unenforced Approve/Publish step over the finalised aggregate. | Registrar; student/guardian for their own (`me.js`, `student/me.js`, already built); teacher, via their own roster | None | Enrolment + 7 years (IT-04 §7.1). Not enforced in code. |
...
| **Certificates** | `registrar/certificates.js`'s `issue` action creates a **pending `staff_approvals` request** (session + Permission Engine, `certificates`/Create); the certificate row itself is only written by the `approve` action, once a distinct Principal decides on it (`docs/approval-workflow-architecture.md`) — the first certificate write path this project has built, and the first place "jointly" in the Matrix is a real, enforced two-step approval rather than a recorded field. | Registrar (requests) + Principal (approves) | No edit path for an issued certificate's core fields (type/reference/date) — only `revoke` exists, mirroring the Ijazah pattern deliberately. | **Now enforced** — `approved_by_staff_id` on the `certificates` row is only ever set by `decideApproval()`'s separation-of-duties + real Permission Engine check, replacing the old unpersisted, unverified `approvedByStaffNo` field. `revoke` itself remains a single REG-held action, unchanged, since the Matrix never gave PRIN a joint grant over it. | Registrar, Principal; student/guardian for their own (`registrar/student.js`'s `certificates` array; guardian/student-facing surfacing not yet built — see "What this register leaves open" below) | None — `revoke` sets `revoked_at`/`revocation_note`, never deletes; `student_id ON DELETE SET NULL` mirrors Ijazah's permanence pattern | Open question, same reasoning as the Ownership Register's entry — arguably permanent given a certificate may be needed decades later. Not yet Board-confirmed. No code-level retention logic (matches the "no purge job" pattern across every record type in this register). |
...
- **Migrating Attendance and Assessments surfaced the same real,
  ...
  rather than a one-off** — and Teacher Identity & Academic Workforce
  Activation has since closed it for the Class Teacher / Subject
  Teacher (`TCH`) half. The Matrix grants REG only Edit/correction on
  both areas; Create belongs to `TCH` (now a real, provisionable role
  with a working `teacher_class_assignments` scope check) and to
  Muhaffiz/Arabic Instructor, neither of which has an issued staff
  account yet — that second half of the gap is named, not resolved,
  see `docs/teacher-portal.md`. See `docs/teacher-operating-model.md`
  for the richer, still-Board-pending role hierarchy this phase did not
  adopt.
