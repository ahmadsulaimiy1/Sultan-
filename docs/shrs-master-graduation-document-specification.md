...
- **Asset — supplied by the client, now on file.** Five real seal images have been provided and are stored at `assets/images/seals/`, never fabricated (§3.6):
  ...
  - `raees-islamic-arabic-studies-seal.jpg` — Office of the Ra'ees, School of Islamic and Arabic Studies.
  ...
  - **No seal yet exists for the Royal College or Nursery & Primary Principal/Head Teacher** — stated plainly rather than substituted with the general seal, so a future Testimonial/Character Certificate for those two schools' students has an honest, named gap instead of a silent stand-in.
...
- **Seal enforcement wired in for real.** `performOnApprove()` calls `requireRealSeal({ role: 'PRIN', institutionName, documentType })` before writing the issued row — exactly the enforcement point the Seal Management Architecture's own comments named these two document types for. In practice today: a Testimonial/Character Certificate for a Royal College or Nursery & Primary graduate is honestly refused at final approval (their Principal/Head Teacher seals remain placeholders per §12), while Islamic & Arabic Studies and Qur'an College graduates' documents issue for real. The approval request itself is never blocked by this — only final issuance — so a Principal can still review and the request stays pending until a real seal is supplied.
...
- `requireRealSeal({ role: 'PRIN', ... })` enforced at issuance, same as Class B — a Certificate for Royal College or Nursery & Primary is honestly refused today until those Principal/Head Teacher seals are real.
