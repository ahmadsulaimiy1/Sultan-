...
- **Asset — supplied by the client, now on file.** Two real seal images were provided and are stored at `assets/images/seals/`, never fabricated (§3.6):
  - `institutional-seal-gold.jpg` — the general institutional seal: an embossed gold medallion carrying the full crest (crescent-and-star, open book, three stars, tree) and "Sultan Hanafi Royal Schools" in Arabic and English.
  - `registrar-office-seal.jpg` — the Office of the Registrar's own ink stamp of the same crest, reading "Office of the Registrar."
  - Both are real photographs (plain/paper background, not yet an isolated cutout — see below), resized to 700px on the long edge and JPEG-compressed for web delivery; the originals as supplied are not altered otherwise.
  - **Which seal appears on which document** is resolved by issuing office, not by document class: a document issued by the Registrar's Office (e.g. the Alumni Registration Certificate, §16) carries `registrar-office-seal.jpg`; documents requiring the Executive/Board's own institutional authority (Board/Founder-CEO Awards, and the Graduation Certificate once built) are the natural fit for `institutional-seal-gold.jpg`. Additional office-specific seals (Principal, Examinations & Records, etc.) remain to be supplied the same way, if and when the client wants document-specific sealing beyond these two.
...
**§20.1 Infrastructure decision — still outstanding.** No real triggered/batch PDF rendering pipeline exists yet; §20.5's remaining document types (Class B, then Class A) remain gated on the client's choice between a dedicated rendering service and a scheduled batch process (§6.2, §22). The seal blocker is now cleared for Registrar-issued and Executive/Board-issued documents; a Class A template (Certificate) built before the infrastructure decision would still only render as interim print-ready HTML, same as Alumni Registration today.
