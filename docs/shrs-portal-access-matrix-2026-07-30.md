...
6. **Manage admissions** — this is the one genuine gap: `functions/api/portal/staff/admissions-applications.js` (list + approve/decline) is fully built and working, but **no staff-facing page calls it yet** — only the guardian's own `/portal/apply/` page and this backend exist. If you want this testable today, say so and it's a small, fast addition (the hard part — the API with permission checks and audit logging — is already done); otherwise it's queued as part of the Registrar reframe (task #440).
