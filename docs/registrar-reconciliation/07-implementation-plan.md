...
> reissue or revocation is performed. Every step below is written in the future tense for
...
|---|---|---|
| P-01 | The **classified workbook** is complete and signed: all 51 rows carry `final_classification` and `recommended_action_code`, and both the `registrar_signoff` and `auditor_signoff` columns are complete (Stage 2 exit criteria). | The signed workbook itself |
...
| P-04 | If any **C1 or B2b** action is on the signed list: key **v3 has been generated** at the Stage 3 ceremony (it does not exist at the evidence baseline — audit §3), its SHA-256 **fingerprint (first 16 hex characters) is recorded** in the ceremony record, and the key is **installed to both custody locations** named in the countersigned custody memorandum (`docs/certificate-key-deployment.md` §3). | The ceremony record; fingerprint checks at both locations |
| P-05 | If any C1 or B2b action is on the signed list: in Cloudflare Pages, **`DOCUMENT_HASH_KEY_VERSION` is `3` and `DOCUMENT_HASH_SECRET` is the v3 key, set together in one save, in Production AND Preview** (`04-sop.md` Stage 3 step 4.4 — setting one without the other would make the live verifier check the six v2 certificates against the wrong key), and **`DOCUMENT_HASH_SECRET_V1` is confirmed still present** — the seven Ibtidā'iyyah certificates (000035–000041) depend on it permanently. | The read-only environment-name audit (`.github/workflows/cloudflare-env-audit.yml`), re-run after the rotation |
...
Before **any** forward action in any batch: run the two read-only audits and record their
run IDs on the forward artefact — that fresh pre-execution snapshot is the state a
rollback must restore (`05-rollback-plan.md` §3.2).
...
| **Inputs from the signed row** | `printed_certificate_no_exact` (character for character), `student_full_name` **as printed**, `student_identity_no` as printed (empty if the paper carries none), `programme_code`, `printed_issue_date`, and the F1 worksheet's recomputed hash. Nothing is normalised, corrected, or reconciled — the paper governs. |
| **Rollback artefact** | The paired `DELETE … WHERE id = <exact id> AND serial_no = '<exact serial>'` of `05-rollback-plan.md` §4.2. Sequence numbers consumed are spent and not reused. |
...
| **Rollback artefact** | The paired `DELETE … WHERE id = <exact id> AND serial_no = '<exact serial>'` of `05-rollback-plan.md` §4.2, drafted with the forward artefact. Rollback is available only until the certificate is printed or announced (the publication boundary, `05-rollback-plan.md` §2). |
...
3. **The recomputed allocation is itself a signed artefact.** It is prepared at the joint
   ...
   reissue runs against an unsigned recomputation.
...
5. **Numbers spent by rolled-back actions stay spent** (`05-rollback-plan.md` §4.2
   step 4). A gap in the numbering costs nothing; a reused number risks the exact
   two-documents-one-number ambiguity the suffix system exists to prevent.
...
1. Re-run the **acceptance** workflow (`.github/workflows/certificate-verification.yml`)
   against production. When dispatched purely as a verification run, its `run_import` and
   `configure_cloudflare` inputs are set as the signed batch definition states — an
   import is a forward action, never a side effect of checking one.
...
halts the batch: the staged rollback is invoked for any partially applied action, the
deviation is logged, and the matter returns through the chain to Stage 2 or Stage 4 as
its nature requires (`04-sop.md` Stage 5 step 4). Execution does not resume on anyone's
initiative at the keyboard.
